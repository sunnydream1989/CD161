...
index_name = c('Leukocyte Fraction','Lymphocyte Infiltration Signature Score')
for (i in 1:length(index_name)){
  ...
  p <- ggviolin(data, 
                ...
                order=c("High", "Low"),
                size=0.5, 
                palette=c("#F5B7B1", "#85C1E9"), #c("#2e9fdf", "#f8766d"), #"aaas", 
                # color="KLRB1_Group", 
                fill="KLRB1_Group", 
                add = "boxplot", 
                width = 0.8, 
    ...
    stat_compare_means(label.x = 1, label.y = ceiling(max(data[index_name[i]], na.rm=TRUE))) + 
...
p <- ggviolin(datasub, 
  ...
  stat_compare_means(aes(group = KLRB1_Group), label.x = 1, label.y = ceiling(max(datasub["abundances"], na.rm=TRUE))) + 
  guides(fill=guide_legend(title=NULL)) + 
  xlab("") + # xlab(indicator_name) +
  ylab("Z score") +
  # labs(title=index_name[i]) +
...
p <- ggviolin(data,
  ...
  stat_compare_means(label.x = 1, label.y = 80) + 
  guides(fill=guide_legend(title=NULL)) + 
